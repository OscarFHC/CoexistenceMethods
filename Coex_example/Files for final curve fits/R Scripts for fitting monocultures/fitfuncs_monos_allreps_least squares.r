...
##### locally defined LS function, uses DATA
 LS = function(par,d) {
    ...
    d = d 
      ...
      least = sum(c((DATA[1:y,jump*species+1]-predict)^2, (DATA[1:y,jump*species+2]-predict)^2, (DATA[1:y,jump*species+3]-predict)^2))                                  
    return(least)
	}
...
maximum =c(max(DATA[1:y,jump*species+1]),   max(DATA[1:y,jump*species+2]), max(DATA[1:y,jump*species+3]))               
        
IC= mean(c(DATA[1,jump*species+1], DATA[1,jump*species+2], DATA[1,jump*species+3]))                   
meank<-mean(maximum)
...
plotmax<-max(c(max(DATA[1:y,jump*species+1]), max(DATA[1:9,jump*species+2]), max(DATA[1:y,jump*species+3])))
...
points(DATA[1:y,1]+1,DATA[1:y,jump*species+2],col=3)
points(DATA[1:y,1]+1,DATA[1:y,jump*species+3],col=4)
points(DATA[1:y,1]+1,rep(50,y),pch=3)
text(19,plotmax+0.1*plotmax,"Tetraedron minimum")


mean<-mean(c(DATA[1:y,jump*species+1], DATA[1:y,jump*species+2], DATA[1:y,jump*species+3]) )                 

RSQ = function(IC,r,k,d) {
		predict = Predict(IC,r,k,d)
    RSS<-sum(c((DATA[1:y,jump*species+1]-predict)^2, (DATA[1:y,jump*species+2]-predict)^2, (DATA[1:y,jump*species+3]-predict)^2))                                    
    rsq<-1-(RSS/sum(c((DATA[1:y,jump*species+1]-mean)^2,(DATA[1:y,jump*species+2]- mean)^2, (DATA[1:y,jump*species+3]-mean)^2 )))                               
    print(rsq)
	}
