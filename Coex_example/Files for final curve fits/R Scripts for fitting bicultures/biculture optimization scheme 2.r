...
Predict <- function(ICa, ICb, ra, rb, ka, kb, aab, aba, d){
...
#### locally defined logl function, uses DATA
  ...
  NLL = function (ICa, ICb, ra, rb, ka, kb, aab, aba, d) {
    ...
    predict1 = Predict (ICa, ICb, ra, rb, ka, kb, aab, aba, d)
...
#preds<-PredictC(ICa, ICb, ra,rb,ka,kb, aab, aba, d=0.1) 
#matplot(DATA[1:y, countera:counterb])
#matpoints(preds)

AAB<- seq (1, 3, by = 1)
ABA<- seq (0.001, 0.005, by = 0.001)
ICA<- exp(seq(log(10000),log(100000),by=log(10)))
ICB<- exp(seq(log(10),log(100),by=log(10)))
