...
ICA<- c(1, 2, 4, 8)
ICB<- c(1, 5, 10, 20)
AAB<- seq(600, 1100, by = 100)
ABA<- seq (-0.01, 0.01, by = 0.005)
...
plot(1:24, fish(23)[ ,1], pch = NA, xlab = "time (days)",
  ...
  max(DATA[1:y, countera], na.rm=T) + 3000))
...
text(23.5, max(DATA[1:y, counterb] ,na.rm = T) +500, toString(bottle))
...
write.csv(goodfitord, "Bottle 100 goodfitord.csv")
write.csv(goodfit5ord, "Bottle 100 goodfit5ord.csv")
goodfit5ord$AIC[1]
b