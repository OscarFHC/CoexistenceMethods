...
AAB<- seq (1, 1.08, by = 0.01)
ABA<- c(0.4, 0.5, 0.6, 0.7)
ICA<- c(40000, 60000, 80000, 100000)
ICB<- c(10, 20, 40, 80, 160)
...
plot(1:24, fish(23)[ ,1], pch = NA, xlab = "time (days)",
  ylab = "Density (cells/mL)", xaxt = 'n', ylim = c(min(DATA[1:y, countera:counterb], na.rm=T)- 1000,
  max(DATA[1:y, countera: counterb], na.rm=T) + 3000))

lines(1:24,fish(23)[,1],col="red")

points(DATA[1:y,1]+1,DATA[1:y,countera],col="red")           
points(DATA[1:y, 1] + 1, DATA[1:y, counterb], col="green")    ###     added to plot on the same axis
lines(1:24,fish(23)[,2], col="green")                         ###   added to plot on the same axis
