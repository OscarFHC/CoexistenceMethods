...
counterb = countera  + 1
y= 12          #overall time series length, e.g. for plotting
ya= 12         #number of points included for time series of species A
yb= 12         #number of days included for time series of species B
...
Predict <- function(ICa, ICb, ra, rb, ka, kb, aab, aba, d){
  ...
  TS_length = 22 # length of time series (TS)
	...
	out[1,1] = ICa
	out[1,2] = ICb
	for(i in 1:TS_length){out [i+1, ] = M_2(out[i,], q)}
...
ICB<- c(100, 500, 1000)
AAB<- c (-250, -350, -450)
ABA<- c (0, 0.01, 0.02, 0.04)

#finalparms = c(ICa = ICa, ICb = ICb, ra = ra, rb = rb, ka = ka, kb = kb, aab = aab, aba = aba, d = 0.1)
#
...
par(mar = c(5, 5, 4, 5))
plot(1:24, fish(23)[ ,1], pch = NA, xlab = "time (days)",
  ylab = "Density (cells/mL)", xaxt = 'n', ylim = c(0, max(DATA[1:y, countera: counterb], na.rm=T) + 3000))
