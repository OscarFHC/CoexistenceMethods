...
DATA <- read.csv ("Competition Model Fit_DT_plus one_biculture 27.csv")
modelday <- DATA [,1]
...
counterb = countera  + 1
y= 18          #overall time series length, e.g. for plotting
ya= 18         #number of points included for time series of species A
yb= 18         #number of days included for time series of species B

######### Generate deterministic time series given parameterss for two species LV competition

Predict <- function(ICa, ICb, ra, rb, ka, kb, aab, aba, d, lag){
  ...
  else {  out [i+1, ] = M_2(out[i,]+ c(1,1), q)}
  }
...
NLL = function (ICa, ICb, ra, rb, ka, kb, aab, aba, d, lag) {

		## likelihood function for a run of data with two species

    predict1 = Predict(ICa, ICb, ra, rb, ka, kb, aab, aba, d, lag)
...
finalparms = c(ICa = ICa, ICb = ICb, ra = ra, rb = rb, ka = ka, kb = kb, aab = aab, aba = aba, d = 0.1, lag = lag)
...
goodfit <- matrix (nrow = length(AAB)*length(ABA)*length(ICA)*length(ICB)*length(LAG), ncol = 10)
    ...
    for (d in 1: length(LAG))   {
        lag = LAG[d]

count = count + 1

      parmsstart = list ("ICa" = ICa, "ICb" = ICb, "aab" = aab, "aba"= aba)
      parmsfixed = list ("ra" = ra, "rb" = rb, "ka" = ka, "kb" = kb, "d" = 0.1, "lag" = lag)
...
colnames(goodfit)=c("aab","aba","ICa","ICb","aabest","abaest","ICaest","ICbest", "lag", "AIC")
...
colnames(goodfit1)= c("aab","aba","ICa","ICb","aabest","abaest","ICaest","ICbest", "lag", "AIC")

########## Nelder-Mead run # 2

goodfit2 <- matrix (nrow = sample, ncol = 10)
    for (j in 1:sample)    {
...
colnames(goodfit2)=c("aab","aba","ICa","ICb","aabest","abaest","ICaest","ICbest","lag", "AIC")

goodfit3 <- matrix (nrow = sample, ncol = 10)
    for (j in 1:sample)    {
...
colnames(goodfit3)=c("aab","aba","ICa","ICb","aabest","abaest","ICaest","ICbest","lag", "AIC")

goodfit4 <- matrix (nrow = sample, ncol = 10)
    for (j in 1:sample)    {
...
colnames(goodfit4)=c("aab","aba","ICa","ICb","aabest","abaest","ICaest","ICbest","lag", "AIC")

goodfit5 <- matrix (nrow = sample, ncol = 10)
    for (j in 1:sample)    {
...
goodfit5[j, 9] = lag
goodfit5[j, 10] = AIC(fit)

print(j)
}

goodfit5 = data.frame(goodfit5)
colnames(goodfit5)=c("aab","aba","ICa","ICb","aabest","abaest","ICaest","ICbest","lag", "AIC")
...
finalparms = c(ICa = goodfit5ord$ICaest[1], ICb = goodfit5ord$ICbest[1],
 ra = ra, rb = rb, ka = ka, kb = kb, aab = goodfit5ord$aabest[1], aba = goodfit5ord$abaest[1], d = 0.1, lag = goodfit5ord$lag[1])
...
fish<- function(x) {
  ...
  else {  out [i, ] = M_2(out[i-1,]+c(1,1), parms)}
  }
	return(out)
  }
...
RSQ = function(ICa, ICb, ra, rb, ka, kb, aab, aba, d, lag) {
		predictf = Predict(ICa, ICb, ra, rb, ka, kb, aab, aba, d, lag)
...
RSQ(as.numeric(finalparms[1]),as.numeric(finalparms[2]),as.numeric(finalparms[3]),
  ...
  as.numeric(finalparms[7]),as.numeric(finalparms[8]),as.numeric(finalparms[9]),as.numeric(finalparms[10]))
  print(finalparms)
  
AIC= goodfit5ord$AIC[1]

write.csv(goodfit5ord,"Bottle 104 timelag output.csv")
