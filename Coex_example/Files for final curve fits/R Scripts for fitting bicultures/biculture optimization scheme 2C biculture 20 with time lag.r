...
PredictC <- function(ICa, ICb, ra, rb, ka, kb, aab, aba, d, lag){
  ...
  if (i < lag+1) {
  out [i+1, 1] = M_2(out[i, ], q)[1]
  out [i+1, 2] = out [i, 2]   
  }
  else {  out [i+1, ] = M_2(out[i,] , q)}
  }
...
NLL = function (ICa, ICb, ra, rb, ka, kb, aab, aba, d, lag) {
    ...
    predict1 = PredictC(ICa, ICb, ra, rb, ka, kb, aab, aba, d, lag)
...
AAB<- (-0.1, -0.08, -0.06, -0.04, -0.02, -0.009, -0.007, -0.005)
ABA<- seq (-5, -15, by = 2)
ICA<- c(200, 300, 400)
ICB<- c(2500, 3000, 3500)
LAG<- c(11, 12, 13)
...
count = count + 1
      ...
      tryCatch( {fit<-mle(NLL, start = parmsstart, method = "Nelder-Mead", fixed = parmsfixed)}, error = function (e) { print("test") } )
...
RSQ = function(ICa, ICb, ra, rb, ka, kb, aab, aba, d, lag) {
		predictf = PredictC(ICa, ICb, ra, rb, ka, kb, aab, aba, d, lag)
