...
PredictC <- function(ICa, ICb, ra, rb, ka, kb, aab, aba, d){
  ...
  TS_length = 22 # length of time series (TS)
	...
	out[1,1] = ICa
	out[1,2] = ICb
	for(i in 1:TS_length){out [i+1, ] = M_2(out[i,], q)+c(1,1)}
...
AAB<- seq (1, 5, by = 1)
ABA<- seq (0.001, 0.005, by = 0.001)
ICA<- exp(seq(log(10000),log(100000),by=log(10)))
ICB<- exp(seq(log(10),log(1000),by=log(10))) 
...
count = count + 1
      ...
      #try( fit<-mle(NLL, start = parmsstart, method = "SANN", fixed = parmsfixed), silent = T)
...
finalparm = c(ICa = goodfit5ord$ICaest[1], ICb = goodfit5ord$ICbest[1],
 ra = ra, rb = rb, ka = ka, kb = kb, aab = goodfit5ord$aabest[1], aba = goodfit5ord$abaest[1], d = 0.1)

finalparm = c(ICa = 100000, ICb = 49, ra = 0.45, rb = 0.4, ka =1557392, kb = 3840, aab = 1.5736, aba = 0.0024, d = 0.1)  
        
finalparms = as.numeric(finalparm)
 ...
 fish<- function(x) {
  ...
  out[2, 2] = as.numeric(finalparms[2])
	for(i in 3:t){out[i, ] = M_2(out[i-1,], parms)}
	return(out)
  }

par(mar = c(5, 5, 4, 5))
plot(1:24, fish(23)[ ,1], pch = NA, xlab = "time (days)", ylab = "Density (cells/mL)",
      xaxt = 'n', ylim = c(min(DATA[1:y, countera:counterb], na.rm=T)- 1000,
...
par(new = T)
plot(1:24, fish(23)[,2], pch = NA, xlab = "time (days)", ylab = "Density (cells/mL)", xaxt = 'n',
  yaxt = 'n', ylim = c(min(DATA[1:y, counterb], na.rm = T),
  max(DATA[1:y, counterb], na.rm = T) + 1000))
...
text(24, max(DATA[1:y, counterb])+1000, toString(bottle))
